        ...
        while not self._stop_event.is_set():
            try:
                ...
                try:
                    ...
                    # Calculate range profile with range axis using selected window
                    ...
                    range_profile, range_axis, detected_points = rp_dsp.calculate_range_profile(
                        processed_data,
                        self.radar_params,
                    ...
                    try:
                        if not self.range_profile_queue.empty():
                            self.range_profile_queue.get_nowait()
                    except Empty:
                        pass
                    
                    # Queue results with proper backpressure
                    try:
                        if all(x is not None for x in [range_profile, range_axis, detected_points]):
