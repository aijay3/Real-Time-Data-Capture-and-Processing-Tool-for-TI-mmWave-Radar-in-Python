...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.range_profile_label.setStyleSheet("color: white;")
        left_plot_layout.addWidget(self.range_profile_label)
        
        # Range Profile view
        self.range_profile_view = GraphicsLayoutWidget(self.centralwidget)
        self.range_profile_view.setObjectName("range_profile_view")
        self.range_profile_view.setBackground('black')
        self.range_profile_view.setStyleSheet("border: 2px solid white;")
        self.range_profile_view.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.range_profile_view.setMinimumSize(400, 300)
        left_plot_layout.addWidget(self.range_profile_view)
        ...
        self.data_display_table.setHorizontalHeaderLabels(['Range (m)', 'Magnitude (dB)'])
        ...
        self.group_peaks_button.setChecked(True)  # Enable by default
        ...
        range_offset_group = QtWidgets.QHBoxLayout()
        self.range_offset_label = QtWidgets.QLabel(self.centralwidget)
        self.range_offset_label.setFont(label_font)
        self.range_offset_label.setText("Range Offset (m): ")
        self.range_offset_label.setStyleSheet("color: white;")
        
        self.range_offset_spin = QtWidgets.QDoubleSpinBox(self.centralwidget)
        self.range_offset_spin.setStyleSheet("""
            QDoubleSpinBox {
                background-color: #333333;
                color: white;
                border: 1px solid #555555;
        ...
        """)
        self.range_offset_spin.setRange(-10.0, 10.0)  # Allow offset from -10m to +10m
        self.range_offset_spin.setSingleStep(0.1)     # Step by 10cm
        self.range_offset_spin.setValue(-1.0)         # Default value to correct 1m->2m issue
        self.range_offset_spin.setDecimals(2)         # Show 2 decimal places
        self.range_offset_spin.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Fixed)
        self.range_offset_spin.setToolTip("Adjust range offset calibration. Negative values move objects farther, positive values move objects closer.")

        range_offset_group.addWidget(self.range_offset_label)
        range_offset_group.addWidget(self.range_offset_spin)
        controls_layout.addLayout(range_offset_group)
        
        # Add controls group box to right side
        self.right_side_layout.addWidget(controls_group_box)
        
        # Channel selection radio buttons
        radio_group_box = QtWidgets.QGroupBox("Channel Selection", self.centralwidget)
        radio_group_box.setStyleSheet("QGroupBox { color: white; border: 1px solid #555555; border-radius: 5px; padding-top: 15px; margin-top: 5px; } QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top center; padding: 0 10px; }")
        radio_layout = QtWidgets.QVBoxLayout(radio_group_box)  # Changed to vertical layout
        radio_layout.setSpacing(5)
        
        # Create and style radio buttons
        radio_font = QtGui.QFont()
        radio_font.setFamily("Arial")
        ...
        QRadioButton:checked {
            background-color: rgba(0, 140, 240, 220);
            border: 1px solid rgba(100, 200, 255, 200);
            color: white;
        }
        QRadioButton:checked:hover {
            background-color: rgba(20, 160, 255, 220);
        }
        """
        
        # Channel radio button group
        self.channel_radio_group = QButtonGroup(self.centralwidget)
        
        # Create radio buttons
        radio_buttons = [
            ("Sum", "channel_radio_sum"),
            ("CH1", "channel_radio1"),
            ("CH2", "channel_radio2"),
            ("CH3", "channel_radio3"),
            ("CH4", "channel_radio4"),
            ("All", "channel_radio_all")
        ]
        
        for i, (text, name) in enumerate(radio_buttons):
            radio = QRadioButton(text, self.centralwidget)
            radio.setFont(radio_font)
            radio.setStyleSheet(radio_style)
            radio.setObjectName(name)
            radio.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
            radio.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Fixed)
            self.channel_radio_group.addButton(radio, i)
            radio_layout.addWidget(radio)
            setattr(self, name, radio)  # Store reference to radio button
        
        # Set Sum as default
        self.channel_radio_sum.setChecked(True)
        
        # Add radio group box to right side
        self.right_side_layout.addWidget(radio_group_box)
        ...
        # Add window options
        self.window_select.addItem("Hamming")  # Default
        self.window_select.addItem("Blackman-Harris")
        ...
        padding_options = [str(2**i) for i in range(5, 11)]  # 32, 64, 128, 256, 512, 1024
        for option in padding_options:
            self.range_pad_select.addItem(option)
        
        # Set default padding values
        self.range_pad_select.setCurrentText("256")
