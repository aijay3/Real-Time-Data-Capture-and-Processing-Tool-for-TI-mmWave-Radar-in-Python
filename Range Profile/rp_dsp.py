...
def calculate_range_profile(radar_data: np.ndarray, radar_params: Optional[RadarParameters] = None,
                          ...
                          window_type: str = 'hamming', range_padding: Optional[int] = None) -> tuple:
    """
    ...
    range_profile = np.abs(np.sum(range_ffts, axis=2))
    ...
    if radar_params is not None:
        ...
        # Correct range resolution based on FFT size
        # R = (c * f_b) / (2 * S), where f_b = k * fs / N_fft
        # R(k) = k * (c * fs) / (2 * S * N_fft)
        # The term (c * fs) / (2 * S * N_fft) is the range resolution.
        range_resolution = (c * sampling_rate) / (2 * freq_slope * num_adc_samples)
        ...
        # Optional: Apply range offset if needed for calibration
        if hasattr(radar_params, 'range_offset') and radar_params.range_offset != 0:
            # Scale the offset based on the ratio of current padding to base padding
            # This ensures the offset is proportional to the FFT size
            scaling_factor = fft_size / radar_params.base_range_padding
            scaled_offset = radar_params.range_offset * scaling_factor
            range_axis += scaled_offset
            print(f"DEBUG: Applied scaled range offset: {scaled_offset:.4f} (base: {radar_params.range_offset}, factor: {scaling_factor})")
            
        max_range = (num_range_bins - 1) * range_resolution
        print(f"DEBUG: Range axis (corrected) - shape: {range_axis.shape}, range_resolution: {range_resolution:.4f}, max_range: {max_range:.4f}, padding: {fft_size}")
    else:
        # Fallback calculation when radar_params not available
        # This part also needs correction to use padding size
        fft_size = padding_sizes[0]
        range_resolution = (c * sampling_rate) / (2 * freq_slope * fft_size)
        range_axis = np.arange(fft_size // 2) * range_resolution
        bandwidth = freq_slope * (num_adc_samples / sampling_rate)  # This is for information, not used in calc
