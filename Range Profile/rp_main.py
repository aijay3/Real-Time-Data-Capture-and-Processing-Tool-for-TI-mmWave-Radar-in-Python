...
def update_figure() -> None:
    ...
    try:
        ...
        try:
            ...
            # Constrain data to valid range for display
            max_range = radar_params.max_range if radar_params else 10
            valid_mask = (range_axis >= 0) & (range_axis <= max_range)
            range_axis_display = range_axis[valid_mask]
            ...
            if channel_idx == 0:  # Sum mode
                ...
                profile_data_db = profile_data_db[valid_mask]
            ...
            if detected_points:
                ...
                detected_points = [pt for pt in detected_points if 0 <= pt[0] <= max_range]
...
def initialize_gui() -> None:
    ...
    # Prevent panning into negative ranges and beyond the physical max
    try:
        vb = plot_widget.getViewBox()
        vb.setLimits(xMin=0, xMax=max_range, yMin=40, yMax=140)
    except Exception:
        pass
    # Set Y-axis range to 0-120 dB
    plot_widget.setYRange(0, 140)
