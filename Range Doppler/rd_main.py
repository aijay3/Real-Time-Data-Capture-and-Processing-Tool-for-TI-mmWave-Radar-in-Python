...
def filter_duplicate_objects(detected_points: List[Tuple[float, float, float]]) -> List[Tuple[float, float, float]]:
    ...
    Groups detections that are close in both range and velocity, then keeps only the
    strongest detection from each group.

    Args:
        detected_points: List of (range, velocity, magnitude) tuples

    Returns:
        List of filtered (range, velocity, magnitude) tuples
    """
    ...
    velocity_threshold = 0.2  # 0.2 m/s - objects with similar velocity are considered the same
    ...
    for i, (range1, velocity1, mag1) in enumerate(sorted_points):
        if i in used_indices:
            continue

        # Start a new group with this point
        current_group = [(range1, velocity1, mag1)]
        used_indices.add(i)

        # Find all other points that belong to the same object
        for j, (range2, velocity2, mag2) in enumerate(sorted_points[i+1:], i+1):
            ...
            # If ranges are very close (within 0.1m), consider them same object regardless of velocity
            if range_diff <= 0.1:
                current_group.append((range2, velocity2, mag2))
                used_indices.add(j)
            # Otherwise use both range and velocity thresholds
            elif range_diff <= range_threshold and velocity_diff <= velocity_threshold:
                current_group.append((range2, velocity2, mag2))
                used_indices.add(j)
...
def update_figure() -> None:
    ...
    try:
        ...
        try:
            ...
            # Use fixed velocity range from -2 to +2 m/s
            img_rd.setRect(QtCore.QRectF(-2, 0, 4, max_range))
            ...
            if detected_points:
                ...
                filtered_points = filter_duplicate_objects(detected_points)

                # Filter stationary objects if clutter removal is enabled
                if processor and processor.remove_clutter:
                    velocity_threshold = 0.03  # Small threshold in m/s to account for noise
                    filtered_points = [point for point in filtered_points if abs(point[1]) >= velocity_threshold]

                # Sort filtered points by range
                filtered_points = sorted(filtered_points, key=lambda x: x[0])

                # Add rows to table
                for range_val, velocity, magnitude in filtered_points:
...
def initialize_gui() -> None:
    ...
    # Set axis ranges - Fixed ranges for both axes
    plot_rd.setYRange(0, max_range)
    plot_rd.setXRange(-2, 2)  # Fixed velocity range from -2 to +2 m/s
