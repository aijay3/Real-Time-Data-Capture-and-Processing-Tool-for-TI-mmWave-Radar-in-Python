...
def calculate_range_doppler(radar_data: np.ndarray, radar_params: Optional[RadarParameters] = None, 
    ...
    if radar_params is not None:
        ...
        freq_slope = radar_params.config_params['freq_slope'] * 1e12  # Convert MHz/μs to Hz/s
        chirp_time = radar_params.config_params['frame_periodicity'] / (radar_params.config_params['num_loops'] * radar_params.config_params['chirps_per_frame']) * 1e-3  # Convert ms to s
        c = 3e8
    else:
        num_adc_samples = 256
    ...
    # Perform 2D FFT for each receiver (keep complex values)
    doppler_fft_array = np.zeros((num_rx, padding_sizes[0], padding_sizes[2]), dtype=np.complex64)
    ...
    for rx in range(num_rx):
        ...
        # Store complex FFT result for coherent summation
        doppler_fft_array[rx] = doppler_fft

    # Coherent summation across receivers
    range_doppler_complex = np.sum(doppler_fft_array, axis=0)

    # Note: Clutter removal for coherently summed range-Doppler data
    # is handled by CFAR detection at the end of the function.
    # The remove_static_clutter function is designed for range profiles,
    # not for 2D range-Doppler maps.

    # Take magnitude after coherent summation
    range_doppler = np.abs(range_doppler_complex)
    
    # Calculate range axis with proper scaling and FFT bin mapping
    if radar_params is not None:
        fft_size = padding_sizes[0]

        # Calculate actual FFT bin size (not theoretical resolution)
        range_bin_size = (c * sampling_rate) / (2 * freq_slope * fft_size)

        num_range_bins = fft_size // 2  # Only positive frequencies

        # Create the range axis using bin size
        range_axis = np.arange(num_range_bins) * range_bin_size

        # Optional: Apply fixed range offset if needed for calibration
        if hasattr(radar_params, 'range_offset') and radar_params.range_offset != 0:
            # Range offset is a fixed physical distance, not scaled by FFT size
            range_axis += radar_params.range_offset

        max_range = (num_range_bins - 1) * range_bin_size
    else:
        fft_size = padding_sizes[0]
        # Convert freq_slope from MHz/μs to Hz/s
        freq_slope_hz_per_s = freq_slope * 1e12  # MHz/μs to Hz/s
        # Calculate actual FFT bin size for default case
        range_bin_size = (c * sampling_rate) / (2 * freq_slope_hz_per_s * fft_size)
        range_axis = np.arange(fft_size // 2) * range_bin_size
    ...
    pri = radar_params.config_params['frame_periodicity'] * 1e-3 \
          / (radar_params.config_params['num_loops'] \
             * radar_params.config_params['chirps_per_frame'])
    doppler_freqs = np.fft.fftshift(np.fft.fftfreq(padding_sizes[2], d=pri))
    if radar_params is not None:
        carrier_freq = radar_params.config_params['start_freq'] * 1e9
        wavelength = c / carrier_freq
        # Calculate velocity in m/s using standard Doppler formula
