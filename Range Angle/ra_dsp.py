...
def suggest_padding_size(data_shape: tuple, radar_params: Optional[RadarParameters] = None,
    ...
    """
    Get FFT padding sizes for range and doppler dimensions only.
    Angle padding is no longer needed with direct beamforming.
    ...
    Args:
        ...
        angle_padding: Deprecated - no longer used with direct beamforming

    Returns:
        List[int]: [range_padding, 0, doppler_padding] (angle dimension removed)
    """
    num_rx, _ = data_shape
    if radar_params is not None:
    ...
    # Angle padding is no longer needed with direct beamforming
    logger.info("Angle padding not needed with direct beamforming approach")
...
def calculate_range_angle(radar_data: np.ndarray, radar_params: Optional[RadarParameters] = None,
                         ...
                         doppler_padding: Optional[int] = None, angle_padding: Optional[int] = None,
                         cfar_params: Optional[dict] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray,
    ...
    # 3. DIRECT BEAMFORMING (No Angle FFT)
    # Apply clutter removal if requested
    if remove_clutter:
        for chirp in range(num_chirps):
            doppler_ffts[:, :, chirp] = remove_static_clutter(doppler_ffts[:, :, chirp])
    ...
    if radar_params is not None:
        ...
        # Use FFT bin count but scale coordinates based on radar parameters
        num_range_bins = padding_sizes[0] // 2  # Must match FFT output size
        # Create range axis that matches FFT bins but scales correctly to physical coordinates
...
def precompute_steering_vectors(angles: np.ndarray, virtual_array_positions: np.ndarray,
                               wavelength: float, d: float, is_azimuth: bool = True) -> np.ndarray:
    """
    Pre-compute steering vectors for all angles to improve performance.

    Args:
        angles: Array of angles in degrees
        virtual_array_positions: Virtual antenna positions
        wavelength: Radar wavelength
        d: Element spacing
        is_azimuth: True for azimuth, False for elevation

    Returns:
        np.ndarray: Pre-computed steering vectors [num_angles, num_antennas]
    """
    num_angles = len(angles)
    num_antennas = virtual_array_positions.shape[0]
    steering_vectors = np.zeros((num_angles, num_antennas), dtype=np.complex64)
    ...
    for a_idx in range(num_angles):
        ...
        steering_vectors[a_idx, :] = np.exp(1j * phase_shifts)

    return steering_vectors

def direct_beamform_range_bin(range_bin_data_avg: np.ndarray, steering_vectors: np.ndarray) -> np.ndarray:
    """
    Perform direct beamforming without FFT using pre-computed steering vectors.

    Args:
        range_bin_data_avg: Complex range bin data averaged across chirps
        steering_vectors: Pre-computed steering vectors [num_angles, num_antennas]

    Returns:
        np.ndarray: Beamformed output for each angle
    """
    # Apply beamforming using pre-computed steering vectors
    beamformed_output = np.abs(np.dot(steering_vectors, np.conj(range_bin_data_avg)))
    return beamformed_output
