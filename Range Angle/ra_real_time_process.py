...
class DataProcessor(th.Thread):
    ...
    def __init__(self, name: str, config: List[int], 
        ...
        self.window_type = 'hamming'  # Default window type (Hamming)
        self.range_padding = 256  # Default range padding
        self.doppler_padding = 64  # Default doppler padding
        self.angle_padding = 181  # Default angle padding (-90° to 90° in 1° steps)
        
        # CFAR parameters
        self.cfar_params = {
            'guard_cells': 2,      # Default guard cells
            'training_cells': 8,   # Default training cells
            'pfa': 0.01,          # Default probability of false alarm
            'group_peaks': True    # Default peak grouping enabled
        }
    ...
    def set_angle_padding(self, padding: int):
        """Set the angle dimension padding size."""
        self.angle_padding = padding
        logger.info(f"Angle padding set to {padding}")
        ...
        while not self._stop_event.is_set():
            try:
                ...
                try:
                    ...
                    range_azimuth, range_elevation, range_axis, azimuth_angles, elevation_angles, azimuth_points, elevation_points = ra_dsp.calculate_range_angle(
                        processed_data,
                        self.radar_params,
                        ...
                        angle_padding=self.angle_padding,
                        cfar_params=self.cfar_params  # Pass CFAR parameters directly
                    )
                    ...
                    try:
                        if all(x is not None for x in [range_azimuth, range_axis, azimuth_angles, azimuth_points]):
                            # Only send azimuth data for now since that's what we're replacing velocity with
                            self.range_doppler_queue.put((range_azimuth, range_axis, azimuth_angles, azimuth_points), timeout=1.0)
                    except Exception as e:
