...
def filter_duplicate_objects(detected_points: List[Tuple[float, float, float]]) -> List[Tuple[float, float, float]]:
    """
    Filter out duplicate detections that likely represent reflections from the same object.

    Groups detections that are close in both range and angle, then keeps only the
    strongest detection from each group.

    Args:
        detected_points: List of (range, angle, magnitude) tuples

    Returns:
        List of filtered (range, angle, magnitude) tuples
    """
    if not detected_points:
        return []

    # Define thresholds for grouping detections (more lenient to catch more duplicates)
    range_threshold = 1.0  # 1.0 meters - objects within this range are considered the same
    angle_threshold = 5.0  # 5.0 degrees - objects with similar angle are considered the same

    # Sort by magnitude (strongest first) to prioritize keeping strong detections
    sorted_points = sorted(detected_points, key=lambda x: x[2], reverse=True)

    filtered_points = []
    used_indices = set()

    for i, (range1, angle1, mag1) in enumerate(sorted_points):
        if i in used_indices:
            continue

        # Start a new group with this point
        current_group = [(range1, angle1, mag1)]
        used_indices.add(i)

        # Find all other points that belong to the same object
        for j, (range2, angle2, mag2) in enumerate(sorted_points[i+1:], i+1):
            if j in used_indices:
                continue

            # Check if this point is close enough to be considered the same object
            range_diff = abs(range1 - range2)
            angle_diff = abs(angle1 - angle2)

            # If ranges are very close (within 0.1m), consider them same object regardless of angle
            if range_diff <= 0.1:
                current_group.append((range2, angle2, mag2))
                used_indices.add(j)
            # Otherwise use both range and angle thresholds
            elif range_diff <= range_threshold and angle_diff <= angle_threshold:
                current_group.append((range2, angle2, mag2))
                used_indices.add(j)

        # From the group, keep only the strongest detection
        if current_group:
            strongest = max(current_group, key=lambda x: x[2])
            filtered_points.append(strongest)

    # Debug logging to see what's being filtered
    if len(filtered_points) < len(detected_points):
        logger.info(f"Filtered {len(detected_points) - len(filtered_points)} duplicate detections")
                ...
                for line in lines:
                    if line.strip().startswith('lvdsStreamCfg'):
                        ...
                        if len(parts) > 3 and parts[3] != '1':
                            print(f"Original lvdsStreamCfg: {line.strip()}")
                            parts[3] = '1'
                            line = ' '.join(parts) + '\n'
                            print(f"Modified lvdsStreamCfg: {line.strip()}")
                        else:
                            print("lvdsStreamCfg dataFmt is already set to 1, no changes made.")
                    f.write(line)
...
def update_figure() -> None:
    ...
    try:
        ...
        try:
            ...
            if azimuth_points:
                # Filter out reflections from same object by grouping nearby detections
                filtered_points = filter_duplicate_objects(azimuth_points)

                # Sort filtered points by range
                filtered_points = sorted(filtered_points, key=lambda x: x[0])

                # Add rows to table
                for range_val, angle, magnitude in filtered_points:
...
def update_plot_axis_limits() -> None:
    """Update plot axis limits when padding parameters change."""
    global plot_rd, img_rd, radar_params

    if not plot_rd or not radar_params:
        return

    try:
        # Get current processor padding settings
        current_padding = processor.range_padding if processor else 256

        # Calculate new max range based on padding
        # The number of range bins depends on the padding size
        num_range_bins = current_padding // 2  # Only positive frequencies
        range_resolution = radar_params.range_resolution
        new_max_range = num_range_bins * range_resolution

        # Update plot axis ranges
        plot_rd.setYRange(0, new_max_range)  # Z-axis (range)
        plot_rd.setXRange(-new_max_range, new_max_range)  # X-axis (cross-range)

        logger.info(f"Updated plot axis limits for range padding {current_padding}: max_range={new_max_range:.2f}m")

        # Update image rectangle if image exists
        if img_rd.image is not None:
            # Create new coordinate system based on updated range
            x_axis = np.linspace(-new_max_range, new_max_range, 100)
            z_axis = np.linspace(0, new_max_range, 100)
            x_range = x_axis[-1] - x_axis[0]
            z_range = z_axis[-1] - z_axis[0]
            img_rd.setRect(QtCore.QRectF(x_axis[0], z_axis[0], x_range, z_range))

    except Exception as e:
        logger.error(f"Error updating plot axis limits: {e}")
...
def initialize_gui() -> None:
    ...
    def on_range_padding_changed(padding_size):
        if processor:
            ...
            # Update plot axis limits when range padding changes
            update_plot_axis_limits()
